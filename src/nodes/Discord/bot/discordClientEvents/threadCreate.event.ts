...
export default async function (client: Client) {
  client.on("threadCreate", async (thread) => {
    try {
      ...
      if (thread.parentId && state.channels[thread.parentId] || state.channels.all) {
        ;[...(state.channels[thread.parentId ? thread.parentId : 0] ?? []), ...(state.channels.all ?? [])].forEach(async (trigger) => {
          if (trigger.type === "thread") {
            ...
            if (reg.test(threadStarter ? threadStarter.content : "")) {
